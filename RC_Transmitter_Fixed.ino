...
void setup() {
  ...
  // --- NRF Setup (TMRh20 RF24 Library - Optimized for Drone) ---
  radio.begin();
  
  // Channel 108 (2.508 GHz) - Must match Flight Controller
  radio.setChannel(108);
  
  // ACK ENABLED - Critical for reliable drone communication
  // ACK ensures packets are received, essential for safety
  radio.setAutoAck(true);
  radio.setAutoAck(0, true); // Enable ACK on pipe 0
  ...
  // Old: (5, 15) = 5 retries, 3.75ms delay - TOO SLOW for drone
  radio.setRetries(3, 1); // 3 retries, 1*250us = 0.25ms delay per retry
  
  // Fast data rate for low latency (250KBPS = best balance)
  radio.setDataRate(RF24_250KBPS);
  
  // Power level - MAX for range (recommend 10uF capacitor on NRF module)
  // If experiencing brownouts, use RF24_PA_HIGH instead
  ...
  Serial.println(F("  TMRh20 RF24 Library - ACK Enabled"));
  Serial.println(F("========================================"));
  Serial.println(F("Channel: 108 | Power: MAX | Rate: 250kbps"));
  Serial.println(F("ACK: ENABLED | Retries: 3 | Delay: 0.25ms"));
...
void loop() {
  ...
  // Send with ACK confirmation (TMRh20 RF24)
  // write() returns true if ACK received, false if failed
  ...
  // Optimized delay for ~200Hz update rate (5ms = perfect for drone control)
  // This gives excellent responsiveness while maintaining reliability
  delay(5); 
}
